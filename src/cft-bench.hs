...
-- ++ should be + in the second case
sqsum [] = 0
sqsum (x:xs) = sqsum xs ++ x * x

-- [] in the first case should be 0
sumList [] = []
sumList (x:xs) = x + sumList xs

-- : in the second case of append should be ++
append x [] = [x]
append x xs = x : xs

digitsOfInt n = if n <=0 then [] else append (digitsOfInt (n / 10)) [n mod 10]

-- f in case statement should be f b
...
-- the 2nd : in the 2nd line should be ++
append [] l = l
append (x:xs) l = x : xs : l

-- replicate (n-1) [x] should be x : replicate (n-1) x
...
-- not . even should be (not . even)
odd1 x = not . even x

-- insert (head lijst) insertionSort (tail lijst) should be
-- insert (head lijst) (insertionSort (tail lijst))
insert :: Int -> [Int] -> [Int]
insert = undefined
